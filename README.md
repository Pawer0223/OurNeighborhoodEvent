...
- jdbc.properties 
  ...
  **context-datasource.xml (PATH: ts/WEB-INF/config/spring/context-datasource.xml)**
  ```porperties
	<bean id="dataSource"
		...
		<property name="url"
			value="jdbc:oracle:thin:@localhost:1521:SID" /> EX) SID = orcl
		<property name="username" value="USER_ID" /> EX) USER_ID = TEST
		<property name="password" value="USER_PW" /> EX) USER_PW = 1234
	</bean>
  ```
